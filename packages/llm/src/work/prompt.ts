...
export function buildWorkPrompt(args: {
  ...
  return [
    ...
    "",
    "GraphJin knowledge pack — read these files with your `" + shellTool + "` tool BEFORE writing any query. The schema, common relationships, and syntax cheat-sheet are all on disk; do NOT run `graphjin cli list_tables` / `describe_table` / `get_query_syntax` / `get_schema_insights` / `get_discovery_schema` — those return bulk dumps already in these files. For targeted schema questions, see the precision tools listed below:",
    ...
    "",
    "Targeted schema queries — when `insights.json` doesn't already cover the table or pair you need:",
